...
import { catalogo, cotacoesMock, promocoes } from "@/lib/mock-data";
...
function CotacoesPage() {
  ...
  const results = catalogo.filter((p) => q && (p.nome.toLowerCase().includes(q.toLowerCase()) || p.code.toLowerCase().includes(q.toLowerCase())));

  const total = items.reduce((s, i) => s + i.qtd * i.preco, 0);

  const addPart = (p: typeof catalogo[number]) => {
    setItems([...items, { code: p.code, nome: p.nome, qtd: 1, preco: p.preco }]);
    setQ("");
  };
  ...
  return (
    <div className="space-y-6">
            ...
            <tbody>
              {cotacoesMock.map((c) => (
                <tr key={c.id} className="border-t border-border hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3">{c.veiculo}</td>
                  <td className="px-4 py-3 text-muted-foreground">{new Date(c.data).toLocaleDateString("pt-BR")}</td>
                  <td className="px-4 py-3 text-right">R$ {c.total.toFixed(2)}</td>
      ...
      <div className="border-t pt-6">
        <h2 className="text-lg font-semibold mb-4">Cotação C-2026-0001 — FR-1001</h2>

        <Card className="mb-4">
          <CardHeader><CardTitle className="text-base">📋 Itens sugeridos pela manutenção</CardTitle></CardHeader>
          <CardContent className="p-0 overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-muted/40 text-xs text-muted-foreground">
                <tr>
                ...
                {items.map((it, i) => (
                  <tr key={i} className="border-t border-border">
                    ...
                    <td className="px-4 py-2 text-center">
                      <Input className="h-7 w-16 mx-auto text-center" type="number" value={it.qtd} onChange={(e) => setItems(items.map((x, j) => j === i ? { ...x, qtd: Number(e.target.value) } : x))} />
                    </td>
        ...
        <Card className="mb-4">
          <CardHeader><CardTitle className="text-base">🔍 Adicionar peças</CardTitle></CardHeader>
            ...
            {results.length > 0 && (
              <div className="grid sm:grid-cols-2 gap-2">
                {results.slice(0, 6).map((p) => (
                  <div key={p.code} className="flex items-center justify-between border border-border rounded-md p-3">
                    <div>
                      <div className="text-sm font-medium">{p.nome}</div>
                      <div className="text-xs text-muted-foreground">{p.code} · {p.categoria} · R$ {p.preco.toFixed(2)}</div>
                    </div>
                    <Button size="sm" onClick={() => addPart(p)}><Plus className="h-3 w-3 mr-1" /> Adicionar</Button>
                  </div>
                ))}
              </div>
            )}
        ...
        <Card className="mb-4 border-info/40 bg-info/5">
          <CardHeader><CardTitle className="text-base flex items-center gap-2"><Lightbulb className="h-4 w-4 text-info" /> Você pode precisar também...</CardTitle></CardHeader>
          <CardContent>
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-3">
              {promocoes.slice(0, 4).map((p) => (
                <div key={p.id} className="border border-info/30 bg-card rounded-md p-3">
                  <div className="text-sm font-medium">{p.nome}</div>
                  <div className="text-sm text-success font-semibold mt-1">R$ {p.promo.toFixed(2)}</div>
                  <Button size="sm" variant="outline" className="w-full mt-2"><Plus className="h-3 w-3 mr-1" /> Adicionar</Button>
                </div>
              ))}
            </div>
          </CardContent>
        ...
        <Card>
          <CardContent className="p-5 space-y-4">
            ...
            <div className="flex gap-2 justify-end flex-wrap">
              ...
              <Button onClick={aprovar} className="bg-success text-success-foreground hover:bg-success/90">✅ Aprovar e Enviar Pedido</Button>
            </div>
          </CardContent>
        </Card>
      </div>
